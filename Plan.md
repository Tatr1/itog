...
1. Отсутствие ТЗ (документации) не позволяет понять, как он должен работать (негативные сценарии) (например, при подготовке к автоматизации выявлено, что при негативном сценарии под сообщением "Ошибка. Банк отказал в проведении операции" скрывается сообщение: "Успешно. Операция одобрена Банком")
